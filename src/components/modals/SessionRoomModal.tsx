...
import TicketAPI, { TicketCreate } from "../../api/TicketAPI";
...
function SessionRoomModal({ show, onClose, sessionId, sessionDate }: Props) {
  ...
  const handleBuy = () => {
    if (!session) return;
    //if (user) {
    const ticketsList: TicketCreate[] = [];
    ...
    TicketAPI.createTickets(session.id, ticketsList)
      .then((res) => {
        console.log(res);
      })
      .catch((error) => {
